var util = require('util'),
    ...
    config = require('../config'),         //文件配置，环境变量等
    multiparty = require('multiparty'),
    needle = require('needle'),
    logger = require('../logger');          //自定义日志文件
  ...
  request(reqOpts, function(err, response, body) {
    if (err) {
      if (err.connect === true) {
        ...
        logger.error('Connection failed (server may be down): ' + reqOpts.url);
      }
      deferred.reject(err);
      return;
    }
...
function _toReqOpts(server, options) {
  ...
  var reqOpts = {
    ...
    timeout: options.timeout || 10000  //  默认10秒   TODO: 涉及第三方调用的接口可能超时..
  }, data = options.data;
  ...
  // 添加requestedBy字段，发送给服务的请求会带上header: X-Requested-By
...
function _toUploadReqOpts(server, options) {
  ...
  var reqOpts = {
    url: _nextServerUrlRR(server, options.url),
    timeout: options.timeout || 5000 , // 默认5秒   TODO: 涉及第三方调用的接口可能超时..
    multipart: true
  }, data = options.data;
  //  添加requestedBy字段，发送给服务的请求会带上header: X-Requested-By
  if (options.requestedBy) {
